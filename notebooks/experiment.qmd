...
Typical loop: edit locally in Neovim, `git push`, rerun the clone/update cell, then rerun the relevant experiment cells here.
...
Remember: autoreload reloads module definitions, but it does not magically mutate objects already created in memory. Recreate your `system`, loaders, optimizer, or config after changing their definitions.
...
```{python}
#@title 4. Config
# Main recommendation: edit defaults in src/piano_modeling/config.py locally,
# then git push and rerun the clone/update + autoreload cells.
cfg = Config()
...
RUN_EXPORT_EXAMPLE = False #@param {type:'boolean'}
